...
Her işlemci ailesi kendi "dilini" konuşur. Bir Intel işlemci için yazılmış makine kodu, bir MIPS işlemcide çalışmaz.
**ISA markaya/aileye özeldir.** Farklı ISA'lar farklı sayıda register'a, farklı adresleme yöntemlerine ve farklı komut setlerine sahiptir.
...
MIPS mimarisinde veriler doğrudan bellekte değil, işlemcinin içindeki çok hızlı yazmaçlarda işlenir. Bu yüzden derleyici, C kodundaki değişkenleri önce bu yazmaçlara atar.
Genellikle kalıcı değişkenler için $s (saved) yazmaçları, ara işlemler için $t (temporary) yazmaçları kullanılır:
 * skor değişkeni -> $s0
 * taban değişkeni -> $s1
 * bonus değişkeni -> $s2
 * ceza değişkeni -> $s3
